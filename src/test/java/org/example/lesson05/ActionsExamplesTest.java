...
import static org.example.lesson05.CustomsWaiters.elementFirstInCollection;
import static org.example.lesson05.utils.ColorMatcher.hasColor;
import static org.example.lesson05.utils.JsUtils.clickWithJs;
...
public class ActionsExamplesTest {
    ...
    //перед всеми тестами будет регистрироваться драйвер
    @BeforeAll
    static void registerDriver() {
        WebDriverManager.chromedriver().setup();
    }

    //перед каждым тестом создаем новый драйвер
    @BeforeEach
    void setupBrowser() {
    ...
    @Test
    void checkInstrumentsPaneColumnsChanging() throws InterruptedException {
        ...
        actions.clickAndHold(driver.findElement(By.xpath("//label[text()='Наименование']/ancestor::tr//span")))
                ...
                .build()
                .perform();
        //вызываем кастомное ожидание
        webDriverWait.until(elementFirstInCollection(By.xpath("//thead[@class='grid-header']//span[@class='grid-header-cell__label']"), "ВЛАДЕЛЕЦ"));

        //так же можно было не реализовывать кастомное ожидание]6 а воспользоваться лямда выражениями
        webDriverWait.until(driver1 -> driver.findElements(By.xpath("//thead[@class='grid-header']//span[@class='grid-header-cell__label']"))).get(0).getText().equals("ВЛАДЕЛЕЦ");

        List<WebElement> headersList = driver.findElements(By.xpath("//thead[@class='grid-header']//span[@class='grid-header-cell__label']"));
        Assertions.assertEquals("ВЛАДЕЛЕЦ", headersList.get(0).getText()); //1

        //используем assert из библиотеки hamcrest (1 и 2 один смысл, разная реализация)
        assertThat(headersList.get(0), hasText("ВЛАДЕЛЕЦ")); //2
    ...
    void checkCellsColorChangingTest() {
        ...
        assertThat(driver.findElement(By.xpath("//tbody[@class='grid-body']/tr[1]"))
                .getCssValue("background-color"), is("rgba(254, 250, 237, 1)"));
        assertThat(driver.findElement(By.xpath("//tbody[@class='grid-body']/tr[1]")), hasColor("rgba(254, 250, 237, 1)"));

    }

    @Test
    void jsExecutor() throws InterruptedException {
